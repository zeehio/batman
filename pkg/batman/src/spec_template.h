	...
	public:
		...
		{    
			...
			{ 
				...
				// find complementary basis - ought to check this cant be done from thinking about transfrom
				int k=ceil(1.1*(pars.p-pars.n));
				vector<vector<double> > ranmat(k,unit);
				for(int it1=0;it1<k;it1++)
				{
					for(int it2=0;it2<pars.p;it2++)
					{
						ranmat[it1][it2]=my_normrnd(0.0, 1.0/sqrt(static_cast<double>(pars.p)), rng); 
					}	
				}
				
				vector<vector<double> > Bran;
				vector<vector<double> > BtBran;
				...
				for(int cit=0;cit<k;cit++)
				{	
					idwt(ranmat[cit], pars.levsize, pars.h_vec, Bunit);
					Bran.push_back(Bunit);
				}	
				for(int cit=0;cit<k;cit++)
				{	
					dwt(Bran[cit],  pars.nlev, pars.h_vec, unit);
					BtBran.push_back(unit);
				}
				
				MMdif(&ranmat, &BtBran, &ranmat);
				
				// Find complementary basis
				
				
				MY_LAPACK_DBL* pam = new MY_LAPACK_DBL[k*pars.p]; 
				for(int it1=0;it1<k;it1++)
					for(int it2=0;it2<pars.p;it2++)
					{
						pam[it2+pars.p*it1]=ranmat[it1][it2];
					}
...
#ifdef HPC
				LAPACKE_dgeqp3(LAPACK_COL_MAJOR, pars.p, k, pam, pars.p, jpvt, tau); 
				LAPACKE_dorgqr(LAPACK_COL_MAJOR, pars.p, k, k, pam, pars.p, tau); 
#else
				...
				MY_LAPACK_INT DIFF=k;
				dgeqp3_(&PARSP, &DIFF, pam, &PARSP, jpvt, tau, &size_work, &lminus1, &info); 
				lsize_work=(int)size_work;
				
				MY_LAPACK_DBL* work = new MY_LAPACK_DBL[lsize_work];
				dgeqp3_(&PARSP, &DIFF, pam, &PARSP, jpvt, tau, work, &lsize_work, &info); 
				delete[] work;
