...
void read_multiplet_data(int lineno, char filename[], opt* option, vector<string> *listname, 
vector<metab_template> *Tems, vector<double> *x);
...
extern "C" {
    ...
    SEXP batman(SEXP filenames, SEXP bn_int, SEXP ito_int, SEXP rr_int, SEXP sno_int, SEXP pBar)  
    {
       ...
       UNPROTECT(1);
        ...
        datamod(&Data,options.div, options.lowerlimit, options.downsample, dppm, &(options.st), &(options.ed), &data, s);
        ...
        read_multiplet_data(lineno, Pfilenames[3], &options, &listn, &TemsU, &x);
        vector<int> metaUsed;
        ...
        if (Tems.size()==0)
        {
            Rprintf("No metabolites with resonances in the region for analysis, exiting ...\n");
            system("PAUSE");
            exit(1);
        }
        char fdirML[3000]={'\0'};
        ...
        *rPercentComplete = 0;
        ...
        // save results
        matrix dataH(dsize);
        datamod(&Data,options.div, options.lowerlimit, 1, dppm, &(options.st), &(options.ed), &dataH, s);
        if (options.saveHR == 1) 
        {
            char fdirdH[3000]={'\0'};
